...
namespace pFactory{
    ...
    class Communicator: public BaseCommunicator {
        public:
            ...
            explicit Communicator(Group* g, T pemptyValue):
                group(g),
    ...
    class MultipleQueuesCommunicator : public Communicator<T> {
        public:
            ...
            inline bool isEmpty(){
                ...
                for(unsigned int threadIdQueue=0;threadIdQueue<nbThreads;threadIdQueue++){
                    ...
                    if (threadIdQueue != threadId){
                        ...
                        else if (queuePointer[threadId] == deque.size())continue; 
                        else return false;
                    }
                }
                return true;
                        ...
                        if(minQueuePointer > 1000){
                            for(unsigned int i = 0;i < minQueuePointer;i++){
                                for (unsigned int j = 0;j < nbThreads;j++){
                                    if (j != threadIdQueue)queuePointer[j]--;
                                }
                                deque.pop_front();
                            }
                        }
            ...
            inline void recvAll(std::vector<T>& dataNotLast, std::vector<T>& dataLast){
                ...
                for(unsigned int threadIdQueue=0;threadIdQueue<nbThreads;threadIdQueue++){
                    ...
                    if (threadIdQueue != threadId){
                        ...
                        if(minQueuePointer == queuePointer[threadId]){//I am a minumum : there are may be dataLast clauses with no copy
                            while(queuePointer[threadId] != minSecondQueuePointer){// warning, that can be equals (severals minimums equals)!
            ...
            */
            inline T recv(){bool nothing = false;return recv(nothing);}
